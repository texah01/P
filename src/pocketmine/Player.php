...
class Player extends Human implements CommandSender, ChunkLoader, IPlayer{
	...
	public static function isValidUserName(?string $name) : bool{
		...
		$len = strlen($name);
		return $lname !== "rcon" and $lname !== "console" and $len >= 1 and $len <= 32 and preg_match("/[^A-Za-z0-9_§ ]/", $name) === 0;
	}
	...
	public function handleLogin(LoginPacket $packet) : bool{
		...
		if(!self::isValidUserName($packet->username)){
			$this->close("", "§l§c[§aAnti§4-§eName§c]§f§a>§fชื่อมีตัวอักษรที่ไม่เหมาะ ต้องตั้งภาษาอังกฤษ และต้องไม่เกิน 32 ตัวอักษร");
		...
		if(count($this->server->getOnlinePlayers()) >= $this->server->getMaxPlayers() and $this->kick("§l§fผู้เล่นเต็มโปรดรอจังหวะในการเข้าเซืฟ", false)){
			return true;
		}
